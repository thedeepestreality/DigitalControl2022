...
from multi_targets import set_traj_control_interp
...
q0 = np.array([state[0] for state in p.getJointStates(body_id, joints_idx)])
print(f'q0: {q0}')
...
# pos1 = [0.7688694517482966, 0.23542471313784263, 0.9163998041187599]
# orient1 = [-0.09784280137887569, 0.9751858740004592, 0.1976765183684914, -0.019060206032616225]
Xd = splines.pybullet_to_homogeneous(pos1, orient1)
print(f'pos0: {(pos0, orient0)}')
print(f'pos1: {(pos1, orient1)}')
qd = np.array(splines.get_inverse_kinematics(pos1, orient0))
print(f'qd: {qd}')

pt1 = { "pos":[-0.2,0,1.57,0,1.57,0], "ts": 2}
pt2 = { "pos":[0,0.2,1.57-0.2,0,1.57,0], "ts": 4}
pt3 = { "pos":[0.2,0,1.57,0,1.57,0], "ts": 6}
pt4 = { "pos":[0,0,1.57,0,1.57,0], "ts": 8}
traj = [pt1, pt2, pt3, pt4]
...
for i in range(0,sz):
    ...
    if t <= 8:
        ...
        qq = set_traj_control_interp(q0, traj, t)
        # print(f"qq: {qq}")
